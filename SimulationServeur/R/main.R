simCount = 0
droppedPerSim = c(1:simCount)

sim = function(pLambda, pMu, pCapacity, pCount, pDuration) {
  # Paramètres
  serverCapacity = pCapacity
  serverCount = pCount
  mu = pMu
  lambda = pLambda
  time.duration = pDuration
  debug = FALSE
  chart = FALSE
  stats = TRUE
  ...
  main <- function() {
    ...
    if (chart) {
      plot(x=timeHistory, y=queueState, type="l", xlab="Temps", ylab="Requêtes dans la queue")
  ...
  main()
  droppedPerSim <<- c(droppedPerSim, sum(dropped))
  simCount = simCount + 1
}
...
plot(y=droppedPerSim[3:length(droppedPerSim)], x=1:(length(droppedPerSim)-2), xlab="Valeur de lambda", ylab="Requêtes abandonnées")
